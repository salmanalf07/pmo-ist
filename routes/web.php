...
Route::group(['middleware' => ['auth:sanctum', config('jetstream.auth_session'), 'verified'], 'prefix' => 'asana'], function () {
    ...
    route::get(
        ...
        function () {
            $project = asanaProject::all();
            foreach ($project as $data) {
                $section = asanaSection::where('asana_id', $data['id'])->orderBy('ref', 'asc')->get();

                $totalTasks = $section->count();
                $completedTasks = $section->where('status', '1')->count();

                // Hitung persentase task yang complete
                $percentageComplete = $totalTasks > 0 ? ($completedTasks / $totalTasks) * 100 : 0;

                $updSection = asanaProject::find($data['id']);
                $updSection->startDate = $section->first()->start_on ?? null;
                $updSection->dueDate = $section->last()->due_on ?? null;
